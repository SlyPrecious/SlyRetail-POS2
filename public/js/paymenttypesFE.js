...
fetch('/currencies')
  ...
  .then(currencies => {
    ...
    //   currencyoption.addEventListener("click", function () {
    //     document.querySelector(".selectPay1").innerText = currencyoption.innerText;
    //     document.querySelector("#select-payment-type").style.display = "none";
    //   });
    // });
      ...
      function saveCurrencies() {
        ...
        if (currencyExist) {
          ...
          fetch('/createNewCurrency', { //THIS IS AN API END POINT TO CARRY THE VARIABLE NAMES TO ANOTHER JS MODULE WHICH WILL BE THE SEVER
            ...
            body: JSON.stringify({ paymentType, paymentName, paymentRate }) // THE VARIABLES ARE STORED AS JOSON OBJECT, TRANSFEREABLE TO SERVER MODULE WHEN THE SERVER TAPS INTOR BODY-PARSER
          })
            .then(response => response.json())
            .then(data => {
      ...
      function UpdateTable() {
        ...
        if (currencyExist && (currencyExist.paymentType).toLowerCase() === (paymentType).toLowerCase() && Number(currencyExist.RATE) === Number(paymentRate)) {
          ...
          fetch('/UpdateCurrencies', {
            ...
            body: JSON.stringify({ paymentType, paymentName, paymentRate, currencyId })
          })
            .then(response => response.json())
            .then(data => {
      ...
      fetch('/currencies')
        ...
        .then(currencies => {
          ...
          for (let i = 0; i < newCurrencies.length; i++) {
            ...
            rateCellDivSpan.innerText = currency.RATE
