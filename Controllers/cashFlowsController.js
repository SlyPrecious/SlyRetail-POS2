...
console.log("theBeforeExpenses:", theBeforeExpenses);
console.log("theBeforeIncome:", theBeforeIncome);
 ...
 //update the categories based on the type selected
        if(typeSelected==='Pay in'){
            ...
            typeSelected='PayOut'
                 ...
                 let categoryExist =await categoriesModel.findOne({ category: 'suspense', Balance: typeSelected  });
               console.log(typeSelected)
               if(!categoryExist){
                try {
                    const categoryEntry = new categoriesModel({ category: 'suspense', CategoryLimit: 0, CategoryLimitRange: '', Balance: typeSelected });
...
//============================================================================================================
// export async function insertCashFlowData(itemsToProcess) {
...
//         if (filteredItemsToProcess.length > 0) {
//             // Loop through filtered items and save to the database
//             for (let a = 0; a < filteredItemsToProcess.length; a++) {
//                 const item = filteredItemsToProcess[a];
//                 try {
//                     const newCashFlow = new CashflowModel(item);
//                     await newCashFlow.save();
//                     // insertedDocuments.push(newCashFlow); // Add to the inserted list
//                     isSaving = true;
...
//     }
// }
export async function insertCashFlowData(req,itemsToProcess, checkTemplateStatus) {
    let filteredItemsToProcess = [];
    let insertedDocuments = [];
    let isSaving = false;
    ...
    let baseCurrency =await currenciesModel.findOne({ BASE_CURRENCY: 'Y' });
        ...
        async function populateArrays(date, shift, invoiceNo, description, currency, category, amount, rate) {
            ...
            if (type === 'Pay in') {
                ...
                // Check if the shift exists 
                const existingCategory = await categoriesModel.findOne({ category: category, Balance: "PayIn" });
                if (!existingCategory) {
                ...
                // Check if the shift exists 
                const existingCategory = await categoriesModel.findOne({ category: category, Balance: "PayOut" });
                if (!existingCategory) {
        ...
        for (let i = 0; i < itemsToProcess.length; i++) {
            ...
            if (checkTemplateStatus === 'slyRetailHeaders') {
                if (data.Id === '') {
                    ...
                    try {
                        // Check if the shift exists 
                        const existingCategory = await categoriesModel.findOne({ category: data.Category });
                        if (!existingCategory) {
                            let payOutCat = {}; //THE NEW DOCUMEN
        ...
        async function saveCategoryToDb(categoryData) {
            try {
                const categoryEntry = new categoriesModel(categoryData);
                try {
